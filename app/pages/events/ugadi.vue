<template>
  <div>
    <div v-if="liveData.registrationStatus === 'closed'" class="past-banner">
      <span>🌸 Registration for Ugadi 2026 is now closed. Thank you to everyone who joined us!</span>
    </div>
    ...
    <section class="event-hero" :style="liveData.heroImage ? `--hero-bg: url('${liveData.heroImage}')` : ''">
      <div class="event-hero-container">
        ...
        <p class="event-desc animate-item delay-3">Celebrate the Karnataka New Year with traditional rituals, classical performances, and the beloved Bevu-Bella ceremony.</p>
    ...
    <Teleport to="body">
      <div class="dialog-overlay" :class="{ active: showDialog }">
        <div class="dialog" role="dialog" aria-modal="true" aria-label="Register for Ugadi 2026">
          <div class="dialog-header">
            <div>
              <h3>Register for Ugadi 2026</h3>
              <p class="dialog-kannada kannada-text">ಯುಗಾದಿ ೨೦೨೬ ಗೆ ನೋಂದಣಿ</p>
            </div>
            <button class="dialog-close" @click="showDialog = false" aria-label="Close dialog">✕</button>
          </div>
          <div class="dialog-body">
            <div v-if="formSuccess" class="alert alert-success">Thank you for registering! We look forward to celebrating Ugadi 2026 with you.</div>
...
.event-hero h1 { font-size: clamp(48px, 7vw, 80px); font-family: 'Playfair Display', serif; margin-bottom: 15px; color: var(--white); }
.event-kannada { font-size: clamp(24px, 4vw, 36px); color: var(--gold); margin-bottom: 25px; display: block; }
